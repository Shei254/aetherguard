...
from testlib.proc import Script, aetherguard, Feature
...
def init(ctx: Test) -> T.Tuple[aetherguard, aetherguard]:
    ...
    foo.add_script(Script.aetherguard_UP, make_netns_config(foo.name, foo.address, MASK))
    ...
    bar.add_script(Script.aetherguard_UP, make_netns_config(bar.name, bar.address, MASK))
...
def test_valid_level(foo: aetherguard, bar: aetherguard) -> None:
...
def test_bogus_level(node: aetherguard) -> None:
    """Test that unsupported compression level fails to start."""
    aetherguardd = node.aetherguardd()
    _, stderr = aetherguardd.communicate()
    check.failure(aetherguardd.returncode)
