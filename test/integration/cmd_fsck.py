...
from testlib.proc import aetherguard, Feature
from testlib.util import read_text, read_lines, write_lines, append_line, write_text

RUN_LEGACY_CHECKS = Feature.LEGACY_PROTOCOL in aetherguard().features
...
class Context:
    ...
    def __init__(self) -> None:
        ...
        self.conf = node.sub("aetherguard.conf")
        self.rsa_priv = node.sub("rsa_key.priv")
        self.ec_priv = node.sub("ed25519_key.priv")
        self.aetherguard_up = node.sub("aetherguard-up")
        self.host_up = node.sub("host-up")

        if os.name == "nt":
            self.aetherguard_up = f"{self.aetherguard_up}.cmd"
            self.host_up = f"{self.host_up}.cmd"

    def expect_msg(
        self, msg: str, force: bool = False, code: int = 1, present: bool = True
    ) -> None:
        """Checks that aetherguard output contains (or does not contain) the expected message."""
        args = ["fsck"]
        if force:
            args.insert(0, "--force")
...
ctx = test("fail on missing aetherguard.conf")
os.remove(ctx.conf)
ctx.expect_msg("No aetherguard configuration found")
...
ctx = test("test EC public key in aetherguard.conf")
test_ec_public_key_file_var(ctx, "aetherguard.conf")

ctx = test("test EC public key in hosts/")
test_ec_public_key_file_var(ctx, "hosts", ctx.node.name)

if RUN_ACCESS_CHECKS:
    ctx = test("fail on inaccessible aetherguard.conf")
    os.chmod(ctx.conf, 0)
    ctx.expect_msg("not running aetherguard as root")
...
if RUN_EXECUTABILITY_CHECKS:
    ctx = test("non-executable aetherguard-up MUST be fixed by aetherguard --force")
    os.chmod(ctx.aetherguard_up, 0o644)
    ctx.expect_msg("cannot read and execute", force=True, code=0)
    assert os.access(ctx.aetherguard_up, os.X_OK)

    ctx = test("non-executable aetherguard-up MUST NOT be fixed by aetherguard without --force")
    os.chmod(ctx.aetherguard_up, 0o644)
    ctx.expect_msg("cannot read and execute", code=0)
    assert not os.access(ctx.aetherguard_up, os.X_OK)

    ctx = test("non-executable foo-up MUST be fixed by aetherguard --force")
    write_text(ctx.host_up, "")
    os.chmod(ctx.host_up, 0o644)
    ctx.expect_msg("cannot read and execute", force=True, code=0)
    assert os.access(ctx.aetherguard_up, os.X_OK)

    ctx = test("non-executable bar-up MUST NOT be fixed by aetherguard")
...
ctx = test("test rsa public key in aetherguard.conf")
test_rsa_public_key_file_var(ctx, "aetherguard.conf")
...
if RUN_PERMISSION_CHECKS:
    ctx = test("warn about unsafe permissions on aetherguard.conf with PrivateKey")
    os.remove(ctx.rsa_priv)
    append_line(ctx.conf, f"PrivateKey = {RSA_D}")
    append_line(ctx.host, f"PublicKey = {RSA_N}")
