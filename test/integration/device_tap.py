...
from testlib.proc import Script, aetherguard
...
def init(ctx: Test, mode: str) -> T.Tuple[aetherguard, aetherguard]:
    """Configure nodes."""
...
def run_tests(ctx: Test, mode: str) -> None:
    ...
    log.info("add aetherguard-up scripts")
    foo.add_script(Script.aetherguard_UP, make_up(foo.name, IP_FOO))
    bar.add_script(Script.aetherguard_UP, make_up(bar.name, IP_BAR))
