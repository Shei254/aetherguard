...
"""Test inviting aetherguard nodes through aetherguard-up script."""
...
def run_tests(ctx: Test) -> None:
    ...
    log.info("bar.aetherguard-up must not exist")
    assert not os.path.exists(bar.sub("aetherguard-up"))

    inv = bar.sub("aetherguard-up.invitation")
...
with Test("invite-aetherguard-up") as context:
    run_tests(context)
