...
def test_rsa(foo: aetherguard) -> None:
...
def test_rsa_nolegacy(foo: aetherguard) -> None:
    """Test command 'generate-rsa-keys' on a nolegacy build."""

    log.info("generate RSA key with nolegacy aetherguard")
    _, err = foo.cmd("generate-rsa-keys", code=1)
    check.is_in("Unknown command", err)


def test_eddsa(foo: aetherguard) -> None:
...
def run_tests(foo: aetherguard) -> None:
    """Run tests."""
