...
def test_sign_errors(foo: aetherguard) -> None:
...
def test_verify(foo: aetherguard) -> None:
    ...
    foo.name = "foo"
    util.write_text(foo.sub("aetherguard.conf"), f"Name = {foo}")
...
def test_verify_errors(foo: aetherguard) -> None:
...
def test_sign_verify(foo: aetherguard, bar: aetherguard) -> None:
