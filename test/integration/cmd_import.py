...
def test_import(foo: aetherguard) -> None:
...
def test_export(foo: aetherguard) -> None:
    ...
    util.write_text(foo.sub("aetherguard.conf"), "")
    _, err = foo.cmd("export", code=1)
    check.is_in("Could not find Name", err)

    os.remove(foo.sub("aetherguard.conf"))
    _, err = foo.cmd("export", code=1)
    check.is_in("Could not open", err)


def test_exchange(foo: aetherguard) -> None:
...
def test_exchange_all(foo: aetherguard) -> None:
    """Run tests for command 'exchange'."""
...
def test_export_all(foo: aetherguard) -> None:
