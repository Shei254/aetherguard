...
def init(ctx: Test) -> T.Tuple[aetherguard, aetherguard]:
    ...
    """
    foo.cmd(stdin=stdin)
    foo.add_script(Script.aetherguard_UP, template.make_netns_config(foo.name, IP_FOO, MASK))
    foo.start()
    ...
    """
    bar.cmd(stdin=stdin)
    bar.add_script(Script.aetherguard_UP, template.make_netns_config(bar.name, IP_BAR, MASK))
...
with Test("ns-ping") as context:
    ...
    bar_node[Script.aetherguard_UP].wait()
