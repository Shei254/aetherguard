...
"""Test aetherguard peer invitations."""
...
def run_port0_test(ctx: Test) -> None:
    """Checks that aetherguard invite fails if called with Port 0 and aetherguardd stopped."""
    foo = ctx.node(init=True)
    _, err = foo.cmd("invite", "bar", code=1)
    check.is_in("Please start aetherguardd", err)
...
def run_invite_test(ctx: Test, start_before_invite: bool) -> None:
    ...
    aetherguardd is started *before* creating invitation, and vice versa.
    """
    foo = init(ctx)
    bar = ctx.node()
...
with Test("fail with Port 0 and aetherguardd not running") as context:
