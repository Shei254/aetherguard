...
def test_invite(foo: aetherguard) -> None:
...
def test_invite_errors(foo: aetherguard) -> None:
    """Test invite error conditions."""

    log.info("invite node with aetherguardd stopped")
...
def test_join_errors(foo: aetherguard) -> None:
    ...
    join = foo.aetherguard("-c", work_dir, "join")
...
def test_broken_invite(ctx: Test) -> None:
    ...
    if not resolve("aetherguard.invalid"):
        log.info("test invitation with an invalid domain")
        url = "aetherguard.invalid/QWNVAevHNSHyMk1qarlZAQOB5swl3Ptu1yGCMSZrzKWpBUMv"
        _, err = bar.cmd("join", url, code=1)
        check.is_in("Error looking up aetherguard.invalid", err)
