...
"""Test that all aetherguardd scripts execute in correct order and contain expected env vars."""
...
from testlib.proc import aetherguard, Script, ScriptType, aetherguardScript
...
def init(ctx: Test) -> T.Tuple[aetherguard, aetherguard]:
...
def wait_script(script: aetherguardScript) -> T.Dict[str, str]:
    """Wait for script to finish and check that it was run by aetherguardd *after* the
    script that was used as the argument in the previous call to this function.

    For example, to check that SUBNET_UP is called after aetherguard_UP:
        wait_script(node[Script.aetherguard_UP])
...
def wait_aetherguard(server: aetherguard, script: Script) -> None:
    """Wait for aetherguard_UP / aetherguard_DOWN and check env vars."""
    log.info("checking aetherguard: %s %s", server, script)
...
def wait_subnet(server: aetherguard, script: Script, node: aetherguard, subnet: str) -> None:
...
def wait_host(server: aetherguard, client: aetherguard, script: ScriptType) -> None:
...
def test_start_server(server: aetherguard) -> None:
    """Start server node and run checks on its scripts."""
    server.cmd("-n", NETNAMES["server"], "start")
    wait_aetherguard(server, Script.aetherguard_UP)
...
def test_invite_client(server: aetherguard, client: aetherguard) -> str:
...
def test_join_client(server: aetherguard, client: aetherguard, url: str) -> None:
...
def test_start_client(server: aetherguard, client: aetherguard) -> None:
...
def test_stop_server(server: aetherguard, client: aetherguard) -> None:
    ...
    log.info("test aetherguard-down")
    wait_aetherguard(server, Script.aetherguard_DOWN)
...
def run_script_interpreter_test(ctx: Test) -> None:
    """Check that aetherguardd scripts run with a custom script interpreter."""
    foo = ctx.node()
    stdin = f"""
        init {foo}
        set Port 0
    ...
    """
    foo_up = foo.add_script(Script.aetherguard_UP)
    foo.cmd(stdin=stdin)
