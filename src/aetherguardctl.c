/*
    aetherguardctl.c -- Controlling a running aetherguardd
...
static void version(void) {
	fprintf(stdout,
	        ...
	        "\n"
	        "aetherguard comes with ABSOLUTELY NO WARRANTY.  This is free software,\n"
...
static void usage(bool status) {
	if(status) {
		...
		fprintf(stdout,
		        ...
		        "  start [aetherguardd options]      Start aetherguardd.\n"
		        "  stop                       Stop aetherguardd.\n"
		        "  restart [aetherguardd options]    Restart aetherguardd.\n"
		        "  reload                     Partially reload configuration of running aetherguardd.\n"
		        "  pid                        Show PID of currently running aetherguardd.\n"
		        ...
		        "\n"
		        "Report bugs to aetherguard@aetherguard-vpn.org.\n",
		        program_name);
	}
}
...
static bool stop_aetherguardd(void) {
	if(!connect_aetherguardd(true)) {
		return false;
	}
	...
	while(recvline(fd, line, sizeof(line))) {
		// wait for aetherguardd to close the connection...
	}
...
bool connect_aetherguardd(bool verbose) {
	if(fd >= 0) {
		fd_set r;
		FD_ZERO(&r);
		FD_SET(fd, &r);
		...
		if(select(fd + 1, &r, NULL, NULL, &tv)) {
			fprintf(stderr, "Previous connection to aetherguardd lost, reconnecting.\n");
			closesocket(fd);
			fd = -1;
		} else {
			return true;
	...
	if((pid == 0) || (kill(pid, 0) && (errno == ESRCH))) {
		fprintf(stderr, "Could not find aetherguardd running at pid %d\n", pid);
	...
	sendline(fd, "%d ^%s %d", ID, controlcookie, aetherguard_CTL_VERSION_CURRENT);
	...
	if(!recvline(fd, line, sizeof(line)) || sscanf(line, "%d %d %d", &code, &version, &pid) != 3 || code != 4 || version != aetherguard_CTL_VERSION_CURRENT) {
		if(verbose) {
...
static int cmd_start(int argc, char *argv[]) {
	if(connect_aetherguardd(false)) {
		if(netname) {
			fprintf(stderr, "A aetherguardd is already running for net `%s' with pid %d.\n", netname, pid);
		} else {
			fprintf(stderr, "A aetherguardd is already running with pid %d.\n", pid);
		}
	...
	if(slash++) {
		xasprintf(&c, "%.*saetherguardd", (int)(slash - program_name), program_name);
	} else {
		c = xstrdup("aetherguardd");
	}
	...
	*/
	...
	if(!pid) {
		...
		setenv("aetherguard_UMBILICAL", buf, true);
		exit(execvp(c, nargv));
	} else {
		close(pfd[1]);
	}
	...
	// A nul-byte right before closure means aetherguardd started successfully.
	bool failure = true;
	uint8_t buf[1024];
	ssize_t len;
...
static int dump_invitations(void) {
	...
	while((ent = readdir(dir))) {
		...
		if(b64decode_aetherguard(ent->d_name, buf, 24) != 18) {
			continue;
		}
...
static int cmd_dump(int argc, char *argv[]) {
	...
	while(recvline(fd, line, sizeof(line))) {
		...
		case REQ_DUMP_NODES: {
			...
			if(n != 22) {
				fprintf(stderr, "Unable to parse node dump from aetherguardd: %s\n", line);
				return 1;
			}
		...
		case REQ_DUMP_EDGES: {
			...
			if(n != 8) {
				fprintf(stderr, "Unable to parse edge dump from aetherguardd.\n");
				return 1;
			}
		...
		case REQ_DUMP_SUBNETS: {
			...
			if(n != 2) {
				fprintf(stderr, "Unable to parse subnet dump from aetherguardd.\n");
				return 1;
			}
		...
		case REQ_DUMP_CONNECTIONS: {
			...
			if(n != 6) {
				fprintf(stderr, "Unable to parse connection dump from aetherguardd.\n");
				return 1;
			}
		...
		default:
			fprintf(stderr, "Unable to parse dump from aetherguardd.\n");
			return 1;
		}
	}
...
static int cmd_top(int argc, char *argv[]) {
	...
	return 0;
#else
	fprintf(stderr, "This version of aetherguard was compiled without support for the curses library.\n");
	return 1;
#endif
}
...
static int cmd_pid(int argc, char *argv[]) {
	...
	if(!connect_aetherguardd(true) || !pid) {
		return 1;
	}
...
char *get_my_name(bool verbose) {
	FILE *f = fopen(aetherguard_conf, "r");

	if(!f) {
		if(verbose) {
			fprintf(stderr, "Could not open %s: %s\n", aetherguard_conf, strerror(errno));
		}
...
// Request actual port from aetherguardd
...
static int cmd_config(int argc, char *argv[]) {
	...
	if(node) {
		...
		snprintf(filename, sizeof(filename), "%s", aetherguard_conf);
	}
	...
	// Silently try notifying a running aetherguardd of changes.
	if(connect_aetherguardd(false)) {
		sendline(fd, "%d %d", CONTROL, REQ_RELOAD);
	}
...
int check_port(const char *name) {
	...
	for(int i = 0; i < 100; i++) {
		...
		if(try_bind(port)) {
			...
			if(!f) {
				fprintf(stderr, "Could not open %s: %s\n", filename, strerror(errno));
				fprintf(stderr, "Please change aetherguard's Port manually.\n");
				return 0;
			}

			fprintf(f, "Port = %d\n", port);
			fclose(f);
			fprintf(stderr, "aetherguard will instead listen on port %d.\n", port);
			return port;
		}
	}
...
static int cmd_init(int argc, char *argv[]) {
	if(!access(aetherguard_conf, F_OK)) {
	...
	if(argc > 2) {
		...
		if(tty) {
			char buf[1024];
			fprintf(stderr, "Enter the Name you want your aetherguard node to have: ");
	...
	FILE *f = fopen(aetherguard_conf, "w");
...
#ifndef HAVE_WINDOWS
	char filename[PATH_MAX];
	snprintf(filename, sizeof(filename), "%s" SLASH "aetherguard-up", confbase);
	...
	if(access(filename, F_OK)) {
		...
		fprintf(f, "#!/bin/sh\n\necho 'Unconfigured aetherguard-up script, please edit '$0'!'\n\n#ifconfig $INTERFACE <your vpn IP address> netmask <netmask of whole VPN>\n");
		fclose(f);
	}
...
static int switch_network(char *name) {
	...
	free(aetherguard_conf);
	free(hosts_dir);
	free(prompt);

	xasprintf(&aetherguard_conf, "%s" SLASH "aetherguard.conf", confbase);
...
static int cmd_network(int argc, char *argv[]) {
	...
	while((ent = readdir(dir))) {
		...
		if(!strcmp(ent->d_name, "aetherguard.conf")) {
			printf(".\n");
			continue;
		}

		char fname[PATH_MAX];
		snprintf(fname, sizeof(fname), "%s/%s/aetherguard.conf", confdir, ent->d_name);
...
static int cmd_sign(int argc, char *argv[]) {
	...
	b64encode_aetherguard(sig, sig, 64);
	ecdsa_free(key);
...
static int cmd_verify(int argc, char *argv[]) {
	...
	*newline++ = '\0';
	...
	if(b64decode_aetherguard(sig, sig, 86) != 64 || !ecdsa_verify(key, newline, len + trailer_len - (newline - data), sig)) {
...
static char *complete_info(const char *text, int state) {
	...
	while(recvline(fd, line, sizeof(line))) {
		...
		if(n != 3) {
			fprintf(stderr, "Unable to parse dump from aetherguardd, n = %d, i = %d.\n", n, i);
			break;
		}
