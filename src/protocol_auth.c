/*
    ...
    Copyright (C) 1999-2005 Ivo Timmermans,
                  2000-2022 Guus Sliepen <guus@aetherguard-vpn.org>
...
bool id_h(connection_t *c, const char *request) {
	...
	if(name[0] == '^' && !strcmp(name + 1, controlcookie)) {
		...
		return send_request(c, "%d %d %d", ACK, aetherguard_CTL_VERSION_CURRENT, getpid());
	}
	...
	if(name[0] == '?') {
		...
		return sptps_start(&c->sptps, c, false, false, invitation_key, c->ecdsa, "aetherguard invitation", 15, send_meta_sptps, receive_invitation_sptps);
	}
	...
	if(c->protocol_minor >= 2) {
		...
		if(c->outgoing) {
			snprintf(label, labellen, "aetherguard TCP key expansion %s %s", myself->name, c->name);
		} else {
			snprintf(label, labellen, "aetherguard TCP key expansion %s %s", c->name, myself->name);
		}
