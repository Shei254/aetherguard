/*
    ...
    Copyright (C) 2014-2022 Guus Sliepen <guus@aetherguard-vpn.org>
...
static void print_aetherguard_cmd(const char *format, ...) ATTR_FORMAT(printf, 1, 2);
static void print_aetherguard_cmd(const char *format, ...) {
	if(confbasegiven) {
...
static void print_new_keys_cmd(key_type_t key_type, const char *message) {
	...
	case KEY_RSA:
		...
		print_aetherguard_cmd("generate-rsa-keys");
		break;

	case KEY_ED25519:
		fprintf(stderr, "You can generate a new Ed25519 keypair with:\n\n");
		print_aetherguard_cmd("generate-ed25519-keys");
		break;

	case KEY_BOTH:
		fprintf(stderr, "You can generate new keys with:\n\n");
		print_aetherguard_cmd("generate-keys");
		break;
	}
}
	...
	for(size_t i = 0; i < total_vars; ++i) {
		if(count[i] > 1 && !(variables[i].type & VAR_MULTIPLE)) {
			fprintf(stderr, "WARNING: multiple instances of variable %s in %s\n",
			        variables[i].name, nodename ? nodename : "aetherguard.conf");
		}
	}
...
static char *read_node_name(void) {
	if(access(aetherguard_conf, R_OK) == 0) {
	...
	if(errno == ENOENT) {
		fprintf(stderr, "No aetherguard configuration found. Create a new one with:\n\n");
		print_aetherguard_cmd("init");
		return NULL;
	}
	...
	if(errno == EACCES) {
		...
		if(uid != 0) {
			fprintf(stderr, "You are currently not running aetherguard as root. Use sudo?\n");
		} else {
...
static bool build_host_conf_path(char *fname, const size_t len) {
	...
	if(!name) {
		fprintf(stderr, "ERROR: aetherguard cannot run without a valid Name.\n");
		return false;
	}
...
static bool check_script_confdir(void) {
	...
	while((ent = readdir(dir))) {
		...
		*dash = 0;
		...
		if(strcmp(fname, "aetherguard") && strcmp(fname, "host") && strcmp(fname, "subnet")) {
			static bool explained = false;
			fprintf(stderr, "WARNING: Unknown script %s" SLASH "%s found.\n", confbase, ent->d_name);

			if(!explained) {
				fprintf(stderr, "The only scripts in %s executed by aetherguard are:\n", confbase);
				fprintf(stderr, "aetherguard-up, aetherguard-down, host-up, host-down, subnet-up and subnet-down.\n");
				explained = true;
			}
...
int fsck(const char *argv0) {
	...
	// Check that aetherguard.conf is readable and read our name if it is.
