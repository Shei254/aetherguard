...
static void timeout_handler(void *data) {
	...
	/*
	         timeout_handler will start after 30 seconds from start of aetherguardd
	...
	*/
	if(sleep_time > 2 * udp_discovery_timeout) {
		...
		/*
		        Do not send any packets to aetherguard after we wake up.
	...
	for list_each(connection_t, c, &connection_list) {
		// control connections (eg. aetherguard ctl) do not have any timeout
		if(c->status.control) {
			continue;
		}
...
int reload_configuration(void) {
	...
	/* Parse some options that are allowed to be changed while aetherguard is running */
