/*
    ...
    Copyright (C) 1998-2005 Ivo Timmermans
                  2000-2018 Guus Sliepen <guus@aetherguard-vpn.org>
...
char *confbase = NULL;          /* base configuration directory for this instance of aetherguard */
bool confbase_given;
...
void make_names(bool daemon) {
	...
	if(netname) {
		xasprintf(&identname, "aetherguard.%s", netname);
	} else {
		identname = xstrdup("aetherguard");
	}

#ifdef HAVE_WINDOWS

	if(!RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\aetherguard", 0, KEY_READ, &key)) {
		if(!RegQueryValueEx(key, NULL, 0, 0, (LPBYTE)installdir, &len)) {
			...
			if(!logfilename) {
				xasprintf(&logfilename, "%s" SLASH "aetherguard.log", confbase);
			}
		}
	...
	if(!confdir) {
		confdir = xstrdup(CONFDIR SLASH "aetherguard");
	}

	if(!confbase) {
		if(netname) {
			xasprintf(&confbase, CONFDIR SLASH "aetherguard" SLASH "%s", netname);
		} else {
			xasprintf(&confbase, CONFDIR SLASH "aetherguard");
		}
	}
