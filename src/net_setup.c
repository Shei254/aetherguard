/*
    ...
    Copyright (C) 1998-2005 Ivo Timmermans,
                  2000-2021 Guus Sliepen <guus@aetherguard-vpn.org>
...
static bool add_listen_address(char *address, bool bindto) {
	...
	for(struct addrinfo *aip = ai; aip; aip = aip->ai_next) {
		...
		// aetherguardd instance will use the same port for all addresses it listens on.
...
void device_enable(void) {
	...
	/* Run aetherguard-up script to further initialize the tap interface */

	environment_t env;
	environment_init(&env);
	execute_script("aetherguard-up", &env);
...
void device_disable(void) {
	environment_t env;
	environment_init(&env);
	execute_script("aetherguard-down", &env);
	environment_exit(&env);
...
static bool setup_myself(void) {
	...
	if(!(name = get_name())) {
		logger(DEBUG_ALWAYS, LOG_ERR, "Name for aetherguard daemon required!");
		return false;
	}
	...
	if(!experimental) {
		logger(DEBUG_ALWAYS, LOG_ERR, "No private key available, cannot start aetherguard!");
		return false;
	}
	...
	if(rsa) {
		...
		if(experimental) {
			...
			logger(DEBUG_ALWAYS, LOG_ERR, "No private keys available, cannot start aetherguard!");
			return false;
		}
	}
...
#else
		logger(DEBUG_ALWAYS, LOG_WARNING, "UPnP was requested, but aetherguard isn't built with miniupnpc support!");
#endif
	}
