/*
    aetherguardd.c -- the main file for aetherguardd
...
#ifdef HAVE_MLOCKALL
		        "  -L, --mlock                   Lock aetherguard into main memory.\n"
#endif
...
static bool parse_options(int argc, char **argv) {
	...
	while((r = getopt_long(argc, argv, "c:DLd::n:so:RU:", long_options, &option_index)) != EOF) {
		...
		case OPT_MLOCK: /* lock aetherguardd into RAM */
...
int main(int argc, char **argv) {
	...
	char *umbstr = getenv("aetherguard_UMBILICAL");
	...
	openlogger("aetherguard", use_logfile ? LOGMODE_FILE : LOGMODE_STDERR);
...
int main2(int argc, char **argv) {
	...
	/* Start main loop. It only exits when aetherguard is killed. */
