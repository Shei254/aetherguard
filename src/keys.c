...
ecdsa_t *read_ecdsa_private_key(splay_tree_t *config_tree, char **keyfile) {
	...
	if(!fp) {
		...
		if(errno == ENOENT) {
			logger(DEBUG_ALWAYS, LOG_INFO, "Create an Ed25519 key pair with `aetherguard -n %s generate-ed25519-keys'.", netname ? netname : ".");
		}
...
rsa_t *read_rsa_private_key(splay_tree_t *config_tree, char **keyfile) {
	...
	if(!fp) {
		...
		if(errno == ENOENT) {
			logger(DEBUG_ALWAYS, LOG_INFO, "Create an RSA key pair with `aetherguard -n %s generate-rsa-keys'.", netname ? netname : ".");
		}
