...
#define aetherguard_ECDSA_INTERNAL
typedef struct {
	uint8_t private[64];
	uint8_t public[32];
} ecdsa_t;
...
ecdsa_t *ecdsa_set_base64_public_key(const char *p) {
	...
	len = b64decode_aetherguard(p, ecdsa->public, len);
...
char *ecdsa_get_base64_public_key(ecdsa_t *ecdsa) {
	...
	b64encode_aetherguard(ecdsa->public, base64, sizeof(ecdsa->public));
...
static bool read_pem(FILE *fp, const char *type, void *vbuf, size_t size) {
	...
	while(fgets(line, sizeof(line), fp)) {
		...
		size_t len = b64decode_aetherguard(line, line, linelen);
