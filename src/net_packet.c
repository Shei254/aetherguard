...
bool send_sptps_data(node_t *to, node_t *from, int type, const void *data, size_t len) {
	...
	if(type == SPTPS_HANDSHAKE || tcponly || (!direct && !relay_supported) || (type != PKT_PROBE && origlen > relay->minmtu)) {
		...
		b64encode_aetherguard(data, buf, len);
...
static length_t choose_initial_maxmtu(node_t *n) {
	...
	/* getsockopt(IP_MTU) returns the MTU of the physical interface.
	   We need to remove various overheads to get to the aetherguard MTU. */
	length_t mtu = ip_mtu;
	...
	logger(DEBUG_TRAFFIC, LOG_INFO, "Using system-provided maximum aetherguard MTU for %s (%s): %hd", n->name, n->hostname, mtu);
	return mtu;
	...
	if(n->mtuprobes < 0) {
		...
		for(;;) {
			...
			/* This magic value was determined using math simulations.
			   ...
			   Since 1407 is just below the range of aetherguard MTUs over typical networks,
			   this fine-tuning allows aetherguard to cover a lot of ground very quickly.
