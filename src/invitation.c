...
static bool get_my_hostname(char **out_address, char **out_port) {
	...
	if(check_id(name)) {
		...
		scan_for_hostname(aetherguard_conf, &hostname, &port);
	}
	...
	if(!port || (is_decimal(port) && atoi(port) == 0)) {
		...
		if(pidfile) {
			...
			fprintf(stderr, "aetherguardd is using a dynamic port and is not running. Please start aetherguardd or set the Port option to a non-zero value.\n");
			goto exit;
		}
	}
	...
	struct addrinfo *ai = str2addrinfo("aetherguard-vpn.org", "80", SOCK_STREAM);
	struct addrinfo *aip = ai;
	static const char request[] = "GET http://aetherguard-vpn.org/host.cgi HTTP/1.0\r\n\r\n";
...
int cmd_invite(int argc, char *argv[]) {
	...
	if(!f) {
		...
		if(connect_aetherguardd(true)) {
			...
			fprintf(stderr, "Could not signal the aetherguard daemon. Please restart or reload it manually.\n");
		}
	} else {
	...
	b64encode_aetherguard_urlsafe(hash, hash, 18);
	...
	b64encode_aetherguard_urlsafe(cookiehash, cookiehash, 18);

	free(fingerprint);

	b64encode_aetherguard_urlsafe(cookie, cookie, 18);
	...
	FILE *tc = fopen(aetherguard_conf, "r");
...
make_names:
	...
	snprintf(filename, sizeof(filename), "%s" SLASH "aetherguard-up.invitation", confbase);
	...
	// Filter first chunk on approved keywords, split between aetherguard.conf and hosts/Name
	// Generate a aetherguard-up script from Ifconfig and Route keywords.
	...
	fclose(f);
	bool valid_aetherguard_up = ifconfig_footer(fup);
	fclose(fup);
...
ask_netname:
	...
	if(ask_netname && tty) {
		...
		if((size_t)snprintf(newbase, sizeof(newbase), CONFDIR SLASH "aetherguard" SLASH "%s", line) >= sizeof(newbase)) {
			fprintf(stderr, "Filename too long: " CONFDIR SLASH "aetherguard" SLASH "%s\n", line);
			goto ask_netname;
		}
...
#ifdef HAVE_WINDOWS
	snprintf(filename2, sizeof(filename2), "%s" SLASH "aetherguard-up.bat", confbase);
#else
	snprintf(filename2, sizeof(filename2), "%s" SLASH "aetherguard-up", confbase);
#endif

	if(valid_aetherguard_up) {
		if(tty) {
			FILE *fup = fopen(filename, "r");

			if(fup) {
				fprintf(stderr, "\nPlease review the following aetherguard-up script:\n\n");
				...
				if(response == 'y') {
					...
					fprintf(stderr, "aetherguard-up enabled.\n");
				} else {
					fprintf(stderr, "aetherguard-up has been left disabled.\n");
				}
			}
		} else {
			if(force) {
				...
				fprintf(stderr, "A aetherguard-up script was generated, but has been left disabled.\n");
			}
		}
	} else {
...
int cmd_join(int argc, char *argv[]) {
	...
	// If a netname or explicit configuration directory is specified, check for an existing aetherguard.conf.
	if((netname || confbasegiven) && !access(aetherguard_conf, F_OK)) {
		fprintf(stderr, "Configuration file %s already exists!\n", aetherguard_conf);
		return 1;
	}
	...
	*slash++ = 0;
	...
	if(!b64decode_aetherguard(slash, hash, 24) || !b64decode_aetherguard(slash + 24, cookie, 24)) {
		goto invalid;
	}
	...
	// Connect to the aetherguard daemon mentioned in the URL.
...
next:
	...
	if(!sptps_start(&sptps, NULL, true, false, key, hiskey, "aetherguard invitation", 15, invitation_send, invitation_receive)) {
		ecdsa_free(hiskey);
		ecdsa_free(key);
		return 1;
	}
