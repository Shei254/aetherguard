/*
    ...
    Copyright (C) 1999-2005 Ivo Timmermans,
                  2000-2017 Guus Sliepen <guus@aetherguard-vpn.org>
...
bool send_req_key(node_t *to) {
	if(to->status.sptps) {
		...
		snprintf(label, labellen, "aetherguard UDP key expansion %s %s", myself->name, to->name);
...
static bool req_key_ext_h(connection_t *c, const char *request, node_t *from, node_t *to, int reqno) {
	...
	if(reqno == SPTPS_PACKET) {
		...
		if(sscanf(request, "%*d %*s %*s %*d " MAX_STRING, buf) != 1 || !(len = b64decode_aetherguard(buf, buf, strlen(buf)))) {
	...
	case REQ_KEY: {
		...
		char *label = alloca(labellen);
		snprintf(label, labellen, "aetherguard UDP key expansion %s %s", from->name, myself->name);
...
bool ans_key_h(connection_t *c, const char *request) {
	...
	if(from->status.sptps) {
		...
		size_t len = b64decode_aetherguard(key, buf, buflen);
